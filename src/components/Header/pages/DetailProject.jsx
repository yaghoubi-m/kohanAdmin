...
export default function DetailProject(){
    ...
    const handleImageSubmit = async (endPoint) => {
        if (uploadedImage) {
            try {
                ...
                formData.append("file", postFile, `detail.${format}`);
                const axiosConfig = {
                    method: 'post',
    ...
    return (
        <div className='d-flex justify-content-center align-items-center gap-4 mt-5'>
            <DropZone title="project detail image" onDrop={handleImageUpload}/>
            <DisplayImage
                url='/api/Project/projectDetailMainHeaderImg'
                onLoadUrl={onLoadImgUrl}
                imgUrl={imgUrl}
                uploadedImage={uploadedImage}/>
            <SubmitBtn url='/api/Admin/CreateProjectDetailHeaderImage' handleImageSubmit={handleImageSubmit}/>
