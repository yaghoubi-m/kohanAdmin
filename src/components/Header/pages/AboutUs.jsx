...
export default function AboutUs() {
    ...
    const handleImageSubmit = async (endPoint) => {
        if (uploadedImage) {
            try {
                ...
                formData.append("file", postFile, `about.${format}`);
                const axiosConfig = {
                    method: 'post',
    ...
    return (
        <div className='d-flex justify-content-center align-items-center gap-4 mt-5'>
            <DropZone title="about us image" onDrop={handleImageUpload}/>
            <DisplayImage
                url='/api/AboutUs/homepageAboutUsMainHeaderImg'
                onLoadUrl={onLoadImgUrl}
                imgUrl={imgUrl}
                uploadedImage={uploadedImage}/>
            <SubmitBtn url='/api/Admin/CreateAboutUsHeaderImage' handleImageSubmit={handleImageSubmit}/>
