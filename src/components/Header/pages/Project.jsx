...
export default function Project(){
    ...
    const handleImageSubmit = async (endPoint) => {
        if (uploadedImage) {
            try {
                ...
                formData.append("file", postFile, `project.${format}`);
                const axiosConfig = {
                    method: 'post',
    ...
    return (
        <div className='d-flex justify-content-center align-items-center gap-4 mt-5'>
            <DropZone title="project image" onDrop={handleImageUpload}/>
            <DisplayImage
                url='/api/Project/projectMainHeaderImg'
                onLoadUrl={onLoadImgUrl}
                imgUrl={imgUrl}
                uploadedImage={uploadedImage}/>
            <SubmitBtn url='/api/Admin/CreateProjectHeaderImage' handleImageSubmit={handleImageSubmit}/>
