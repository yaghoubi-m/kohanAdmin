...
export default function Catalog() {
    ...
    const handleImageSubmit = async (endPoint) => {
        if (uploadedImage) {
            try {
                ...
                formData.append("file", postFile, `catalog.${format}`);
                const axiosConfig = {
                    method: 'post',
    ...
    return (
        <div className='d-flex justify-content-center align-items-center gap-4 mt-5'>
            <DropZone title="catalog image" onDrop={handleImageUpload}/>
            <DisplayImage
                url='/api/Catalog/GetCatalogsMainHeaderImage'
                onLoadUrl={onLoadImgUrl}
                imgUrl={imgUrl}
                uploadedImage={uploadedImage}/>
            <SubmitBtn url='/api/Admin/CreateCatalogMainImage' handleImageSubmit={handleImageSubmit}/>
