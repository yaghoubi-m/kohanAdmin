...
export default function ContactUs() {
    ...
    const handleImageSubmit = async (endPoint) => {
        if (uploadedImage) {
            try {
                ...
                formData.append("file", postFile, `contact.${format}`);
                const axiosConfig = {
                    method: 'post',
    ...
    return (
        <div className='d-flex justify-content-center align-items-center gap-4 mt-5'>
            <DropZone title="contact us image" onDrop={handleImageUpload}/>
            <DisplayImage
                url='/api/ContactUs/GetContactUSMainHeaderImg'
                onLoadUrl={onLoadImgUrl}
                imgUrl={imgUrl}
                uploadedImage={uploadedImage}/>
            <SubmitBtn url='/api/Admin/CreateContactUsMainImage' handleImageSubmit={handleImageSubmit}/>
